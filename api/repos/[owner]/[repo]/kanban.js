...
// Uses dynamic import to load the CommonJS parser safely.
...
export default async function handler(req, res) {
  try {
    ...
    // Dynamically import parser relative to this file; supports CommonJS modules.
    const parserUrl = new URL('../../../../src/bitacoraParser.js', import.meta.url).href
    const parserMod = await import(parserUrl)
    // parser may be CommonJS exported as default, or named export
    const parseBitacora = parserMod.parseBitacora || (parserMod.default && parserMod.default.parseBitacora) || (parserMod.default) || null
    if (!parseBitacora) {
      console.error('parseBitacora not found in module', Object.keys(parserMod))
